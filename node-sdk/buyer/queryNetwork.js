  ...
  invokeQuery: async function (request) {
    ...
    try {
      ...
      let user_from_store = await Fabric_Client.newDefaultKeyValueStore({
        ...
        return fabric_client.getUserContext('buyer', true);
      });

      if (user_from_store && user_from_store.isEnrolled()) {
        console.log('Successfully loaded buyer from persistence');
      } else {
        throw new Error('Failed to get buyer.... run registerUserNetwork.js');
      }
